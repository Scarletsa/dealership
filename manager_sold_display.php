...
foreach ($arr as $key=>$emp) {
  $id = '"'.$arr[$key]['empID'].'"';
  $stmt = "SELECT COUNT(empID), SUM(priceSold) FROM $soldTable WHERE `empID` = $id";
  $result = $dbh->query($stmt);
  $temp = $result->fetchall(PDO::FETCH_ASSOC);
  $arr[$key]['numSold'] = $temp[0]['COUNT(empID)'];
  $arr[$key]['totalSold'] = $temp[0]['SUM(priceSold)'];
  if ($arr[$key]['numSold'] > $_SESSION['mostCars'] && $arr[$key]['totalSold'] > $_SESSION['mostDollars']) {
    $_SESSION['mostCars'] = $arr[$key]['numSold'];
    $_SESSION['mostDollars'] = $arr[$key]['totalSold'];
